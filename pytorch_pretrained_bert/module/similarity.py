...
class Bilinear(nn.Module):
    ...
    def forward(self, x, y):
        ...
        scores = x.bmm(proj.contiguous().transpose(1, 2))
        return scores
...
class AttentionWrapper(nn.Module):
    def __init__(self, x1_dim, x2_dim, x3_dim=None, prefix='attention', opt={}, dropout=None):
        ...
        if dropout:
            ...
            self.dropout = DropoutWrapper(self.att_dropout)
    ...
    def forward(self, query, key, value=None, key_padding_mask=None, return_scores=False):
...
class DualAttentionWrapper(nn.Module):
    ...
    def forward(self, query, key, query_padding_mask=None, key_padding_mask=None, return_scores=False):
        ...
        if return_scores:
            return attn_query, attn_key, prob, logits
        else:
...
class SelfAttnWrapper(nn.Module):
    def __init__(self, input_size, prefix='attn_sum', opt={}, dropout=None):
        ...
        if attn_type == 'mlp':
            self.att = MLPSelfAttn(input_size, opt, prefix, dropout)
        else:
